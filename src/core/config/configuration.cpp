...
bool Configuration::outputConfig(const std::string& filePath, EditorWindow *editor) {
    ...
    configOutput << "/*\n";
    configOutput << "This configuration file was generated by ProffieConfig " VERSION ", created by Ryryog25.\n";
...
void Configuration::tryAddInjection(const std::string& buffer, EditorWindow *editor) {
    ...
    std::string filePath{CONFIG_DIR};
    filePath += INJECTION_STR;
    filePath += injectionFile;

    std::error_code err;
    if (not std::filesystem::exists(filePath, err)) {
