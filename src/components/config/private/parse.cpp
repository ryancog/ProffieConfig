...
optional<string> Config::parse(const filepath& path, Config& config, Log::Branch *lBranch) {
    ...
    if (config.propSelection != -1) {
        const auto& dataMap{config.prop(config.propSelection).dataMap()};
        const auto& customOpts{config.settings.customOptions()};
        for (auto idx{0}; idx < customOpts.size(); ++idx) {
            auto& opt{config.settings.customOption(idx)};
            ...
            config.settings.removeCustomOption(idx);
            --idx;
        }
    }
